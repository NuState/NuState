...
export async function GET({url, fetch}: { url: URL, fetch: typeof window.fetch }) {
    ...
    let find: ElasticsearchResponse<Cj>

    try {
        find = await (await fetch(encodeURI(`https://${environment.engines.endpointKey}/api/as/v1/engines/search-code-juridiques/search?query=${q}`), {
            method: 'POST',
            headers: {
